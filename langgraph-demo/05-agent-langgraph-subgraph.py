...
def format_report_for_slack(state):
	...
	return {"report":formatted report} # 返回包含格式化报告的字典
	
	
# 创建问题总结的状态图
qs_builder = StateGraph(QuestionSummarizationState)
qs_builder.add_node("generate_summary", generate_summary
qs_builder.add_node("send to_slack", send_to_slack)
...
question_answer = Logs(
	id="1",
	question="如何导入Chat0penAI?",
	answer="要导入ChatOpenAI,使用:'from langchain_openai import chatOpenAl.'",
)
...
entry builder.add_node("failure_analysis", fa_builder.compile())
...
entry_builder.add_edge("convert_logs_to_docs",
