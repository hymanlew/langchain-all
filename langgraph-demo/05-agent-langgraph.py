...
# 一个 tool_executor，理解为 Acting 的过程。
def tool_executor():
...
#- 2，Normal Edge, 普通边，表示从一个节点到另一个节点的边 model节点到tools节点
...
#定义节点
#判定函数，如果没有tool_calls就结束，如果有就返回tools
def should_continue(state: MessagesState):
	messages = state['messages']
	#获取最后一条消息:从 messages 列表中获职最后一条消息
	last_message = messages[-1]
	#如果最后一条消息包含工具调用，则返回字符串"tools"
