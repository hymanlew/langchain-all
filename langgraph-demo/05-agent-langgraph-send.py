...
builder.add_node("generate_joke", lambda state: {"jokes": [f"Joke about {state['subject']}"]}, retry=RetryPolicy(max_attempts=4))
...
result = graph.invoke({"subjects": ["cats","dogs"]}, config={"recursion_limit":25})
