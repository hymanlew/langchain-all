"""
多 Agent 协作的方式适用于简单逻辑的，或少量 AGENT 的任务协作，当有大量任务 agent 时，共享路由就很麻烦。这时就需要用到主管 AGENT。

主管多 Agent（Multi-agent supervisor）
参考: https://github.com/langchain-ai/langgraph/blob/main/docs/docs/tutorials/multi_agent/agent_supervisor.ipynb

在这个例子中，多个 Agent 连接在一起，但它们不共享一个暂存器。相反，它们有自己独立的暂存器，然后它们的最终响应被附加到全局暂存器中。
...
# 注释掉未定义的 MultiServerMCPclient 类
...
def agent_node(state, agent, next):
	...
	return {
		"messages": [result["messages"][-1]], "next": "supervisor"}
...
system_prompt = (
	...
	"respond with the worker to act next. Each worker will perform a task and respond with their results and status. When finished, respond with end. " +
...
class routeResponse(BaseModel):
	"""
	   ...
	   class routeResponse(BaseModel):
	       next: NodeOptions
	"""
	# Literal的类型参数必须是字符串、字节、整数、布尔值等字面量，或者由它们组成的元组，但不能是动态计算的
    # Literal的类型参数必须是明确的，不能是变量
	next: Literal["end", "research", "coder", "travel_node", "joke_node", "talk_node", "other_node"]
...
def other_node(state):
	...
	return {
		"messages": [HumanMessage(content="我暂时无法回答这个问题")], "next": "supervisor"
	}
...
def talk_node(state):
	...
	prompt = prompt_template.invoke({"samples": samples, "text": state["messages"][-1].content})
...
workflow.add_node("talk_node", talk_node)
...
for member in members:
	...
	if member not in ["travel_node", "joke_node", "talk_node", "other_node", "research", "coder"]:
		continue
